...
export function createSocketComm(inParentComponent: React.Component) {
  ...
  connection.onmessage = function(inMessage: any): void {
  ...
  (createSocketComm as any).send = function(inMessage: string): void {
  ...
  // Return a reference to this function to the caller so that the send() method can be used.
  return createSocketComm;
